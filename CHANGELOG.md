...
- A `:reset_sequences` option (default: `false`) on `Blink.Seeder.run/3`, `Blink.copy_to_table/4`, and `Blink.Adapter.Postgres`, also settable per-table. After a table's copy it advances the sequence behind each `serial` or identity primary key column past the highest copied value, so the application's next ordinary insert no longer collides with a seeded row. Primary keys without a sequence (`uuid`, self-managed integers) are skipped. This replaces the `setval` query the Getting Started guide told you to run by hand.

defmodule Blink.Adapter do
  ...
  Adapters are responsible for implementing database-specific bulk copy operations.
  Each adapter must implement the `call/4` callback which performs the bulk insertion
  using the appropriate database-specific mechanism (e.g., PostgreSQL's COPY,
  MySQL's LOAD DATA INFILE, etc.).
